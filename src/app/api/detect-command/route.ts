...
Return STRICT JSON:
{"isCommand": boolean, "confidence": 0..1, "name": string, "message": string, "codingAgent": "claude"|"codex"|"devin"}

- isCommand: true only if there is a clear directive to spin up / start / launch an agent to build/do something, OR to send/use Devin to build/fix/work on something.
...
- codingAgent: "devin" if the speaker names Devin (e.g. "send Devin to…", "have Devin build…"); otherwise "codex" only if they explicitly ask for codex; otherwise "claude".
  ...
  for (const model of MODELS) {
    try {
      ...
      return NextResponse.json({
        ...
        codingAgent:
          parsed.codingAgent === "devin"
            ? "devin"
            : parsed.codingAgent === "codex"
              ? "codex"
              : "claude",
      });
    } catch (err) {
